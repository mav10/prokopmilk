...
namespace WebSite.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber=model.PhoneNumber, name=model.name, city=model.city, adres=model.adres };
                ...
                if (result.Succeeded)
                {
                    ...
                    client.Phone = model.PhoneNumber;
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "Восстановление пароля", "Для того чтобы сбросить старый пароль перейдите по <a href=\"" + callbackUrl + "\">ссылке</a>");
